...
def process_prompts(paper_indexes, thread_id, papers_matched):
    ...
    for paper_index in paper_indexes:
        try:
            ...
            confnavigator_logger.info(f"Took {time_taken} s to process paper - {paper_index}")
...
def call_openai_api_to_summarize_paper(parsed_summary_file, n_threads: int = 3, openai_model_type: str = "gpt-3.5-turbo-1106"):
    ...
    confnavigator_logger.info(f"Going to call openai api for {len(paper_indexes)} papers")

    # Calculate the size of each chunk
    if len(paper_indexes) < n_threads:
        chunk_size = 1  # Assign one prompt per thread when there are fewer prompts than threads
    else:
        chunk_size = -(-len(paper_indexes) // n_threads)  # Using ceiling division for more even distribution
    ...
    if len(paper_indexes) > 10:
        print("-------------"* 10)
        print(f"It's going to run summary for {len(paper_indexes)} with {openai_model_type}, are you sure that you want to proceed? Press c for continue and press q to quit.")
        print("-------------"* 10)
        breakpoint()
    ...
    for i in range(n_threads):
        thread = threading.Thread(target=process_prompts, args=(paper_indexes_chunks[i], i, papers_matched))
    ...
    end_time = time.time()
    time_taken = end_time - start_time
    confnavigator_logger.info(f"{n_threads} threads cost {time_taken} s to process {len(paper_indexes)} prompts")


    confnavigator_logger.info(f"Extract {response_queue.qsize()} responses from the queue")
    ...
    while not response_queue.empty():
        try:
            ...
            response_summary[paper_title]["prompt_tokens"] = response.usage.prompt_tokens
            input_total_tokens += math.ceil(response.usage.prompt_tokens/1000) * 1000
            response_summary[paper_title]["completion_tokens"] = response.usage.completion_tokens
            output_total_tokens += math.ceil(response.usage.completion_tokens/1000) * 1000
            response_summary[paper_title]["total_tokens"] = response.usage.total_tokens
    ...
    confnavigator_logger.info(f"Total input tokens is {input_total_tokens} and total output tokens is {output_total_tokens}. Total Cost is {total_cost}")
    ...
    summary_paper_json_file_path = os.path.join(output_folder_path, f"Summary_selected_papers.json")
...
if __name__ == '__main__':
    ...
    summary_paper_json_file_path = call_openai_api_to_summarize_paper(parsed_summary_file)
