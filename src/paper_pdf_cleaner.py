...
def convert_arxiv_csv_to_json(csv_file_path):
    ...
    utils.dump_json_file(data=paper_metadata_dict,
                         json_file_path=arxiv_json_file)
...
def paper_pdf_cleaning(arxiv_paper_pdf_folder, openreview_paper_pdf_folder,
                       csv_file_path, combined_paper_pdf_folder,
                       output_folder):
    ...
    for paper_id, paper_data in paper_csv_json.items():
        # !@#$%^&*()-+?_=,<>/"
        correct_title = paper_data["title"].replace(":", "_").replace(
            "'", "_").replace("?", "_").replace("\\", "_")

        arxiv_pdf_file_path = os.path.join(arxiv_paper_pdf_folder,
                                           f'{correct_title}.pdf')
        ...
        openreview_pdf_file_path = os.path.join(openreview_paper_pdf_folder,
                                                f'{correct_title}.pdf')
        ...
        pdf_file_to_parse = os.path.join(combined_paper_pdf_folder,
                                         f'{correct_title}.pdf')
        if paper_data["arxiv_pdf_path"]:
            ...
            if not os.path.isfile(pdf_file_to_parse):
                shutil.copy(paper_data["openreview_pdf_path"],
                            pdf_file_to_parse)
            paper_data["pdf_file_to_parse"] = pdf_file_to_parse
            combined_pdf += 1
        else:
    ...
    papers_overview['arxiv_total_downloaded_papers'] = len(
        utils.find_all_pdf_files_in_folder(arxiv_paper_pdf_folder))

    papers_overview['arxiv_found_pdf_papers_based_on_title'] = valid_arxiv_pdf

    papers_overview['openreview_total_downloaded_papers'] = len(
        utils.find_all_pdf_files_in_folder(openreview_paper_pdf_folder))

    papers_overview[
        'openreview_found_pdf_papers_based_on_title'] = valid_openview_pdf
