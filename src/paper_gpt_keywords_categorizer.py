    ...
    for paper_data in papers_chunk:
        try:
            ...
            if number_tokens >= 16385:
                ...
                prompt = create_categorize_prompt_based_on_abstract(paper_data, ratio)
            else:
                ratio = 1.0
            ...
            while (counter_repeat_gen<3 and not filtered_keywords_set):
                ...
                cleaned_categories = ', '.join([word for word in categories.split(';') if word.strip()])
                print('*** Paper ID= ', paper_data['id'], ' | ', f"Model = {openai_model_type} | ", cleaned_categories)
                categories_set = set([item.strip() for item in cleaned_categories.split(',')])
            ...
            if counter_repeat_gen < 3:
                filtered_keywords_str = ', '.join(str(item) for item in list(filtered_keywords_set)) 
            ...
            category_response_queue.put((thread_id, prompt, paper_data['id'], response, data))
    ...
    while True:
        if not category_response_queue.empty():
            try:
                ...
                categorized_papers_to_record[paper_index]["Keywords_GPT3.5"] = data['Keywords_GPT3.5']
                categorized_papers_to_record[paper_index]["Keywords_GPT3.5_filtered"] = data['Keywords_GPT3.5_filtered']

                categorized_papers_to_record[paper_index]["keywords_response"] = str(response)
                categorized_papers_to_record[paper_index]["keywords_prompt"] = prompt
                ...
                categorized_papers_to_record[paper_index]["keywords_thread_id"] = thread_id
                utils.dump_json_file(categorized_papers_to_record, gpt_keywords_tmp_file)
...
def call_openai_api_to_categorize_paper(
    ...
    categorized_papers_to_record_tmp = {} # Pass into the thread to use the info
    ...
    paper_indexes_chunks = list(divide_chunks(papers_to_categorize, chunk_size))
    ...
    for i in range(n_threads):
        thread = threading.Thread(target=process_papers_chunk,
                                  args=(paper_indexes_chunks[i], i, openai_model_type))
    ...
    queue_thread = threading.Thread(target=process_queue,
                                    args=(categorized_papers_to_record, gpt_keywords_tmp_file))
    queue_thread.start()
    ...
    for paper_index, paper_data in categorized_papers_to_record.items():
        ...
        elif "Keywords_GPT3.5_filtered" in gpt_keywords_info[paper_index] and gpt_keywords_info[paper_index]["Keywords_GPT3.5_filtered"] == "UNKNOWN" and paper_data.get("Keywords_GPT3.5_filtered", "") != "UNKNOWN":
            gpt_keywords_info[paper_index] = paper_data

            keywords_paper += 1
        

    confnavigator_logger.info(f"categorized {len(categorized_papers_to_record)} papers.")
...
def integrate_gpt_keywords(paper_summary_info_file, gpt_keywords_library_file, paper_keywords_info_file):
    paper_summary_info = utils.read_json_file(paper_summary_info_file)
    gpt_keywords_info = utils.read_json_file(gpt_keywords_library_file)

    for paper_index, paper_info in paper_summary_info.items():
        if "Keywords_GPT3.5_filtered" in paper_info and paper_info["Keywords_GPT3.5_filtered"] != "UNKNOWN"  or paper_index not in gpt_keywords_info:
            continue
        
        if "Keywords_GPT3.5" not in gpt_keywords_info[paper_index]:
            continue
        paper_info["Keywords_GPT3.5"] = gpt_keywords_info[paper_index]["Keywords_GPT3.5"]
        paper_info["Keywords_GPT3.5_filtered"] = gpt_keywords_info[paper_index]["Keywords_GPT3.5_filtered"]
        paper_info["keywords_prompt_tokens"] = gpt_keywords_info[paper_index]["keywords_prompt_tokens"]
        paper_info["keywords_completion_tokens"] = gpt_keywords_info[paper_index]["keywords_completion_tokens"]
...
def paper_gpt_categorizing(output_folder):
    ...
    assert os.path.isfile(paper_summary_info_file) == True, f"The provided {paper_summary_info_file} could not be found!"
    paper_summary_info = utils.read_json_file(paper_summary_info_file)

    gpt_keywords_library_file = call_openai_api_to_categorize_paper(paper_summary_info=paper_summary_info, gpt_keywords_library_file=gpt_keywords_library_file, openai_model_type="gpt-3.5-turbo-1106")


    # Combine the keywords info from gpt_keywords_library.json into paper keywords info.
    paper_keywords_info_file = integrate_gpt_keywords(paper_summary_info_file=paper_summary_info_file, gpt_keywords_library_file=gpt_keywords_library_file, paper_keywords_info_file=paper_keywords_info_file)
